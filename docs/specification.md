...
**Changelog:**
- 0.5.1 (2026-05-03): UX corrections to RULE-MCPAUTH-* per AIR-706 review: rewrite fixHints for TOKEN_INVALID, PILOT_NOT_ON_MANIFEST, and SEAT_INSUFFICIENT_AUTHORITY (RULE-UXR-2); rename `crew` authority level to `officer` in §4.10.3 (RULE-UXR-1); add `ruleId` field to RULE-MCPAUTH-8 structured error format for traceability parity with RULE-MCP-2; add §4.9.5 cross-reference clarifying auth-layer vs tool-layer error shapes (AIR-718).
...
- **RULE-MCPAUTH-8:** All authentication and authorization errors from the standalone MCP server MUST include four fields: `code` (machine-readable error code), `ruleId` (the `RULE-MCPAUTH-*` identifier that governs this error, providing traceability parity with the RULE-MCP-2 format), `message` (human-readable description), and `fixHint` (actionable next step for the caller). The `ruleId` field ensures consumers can trace any auth error back to the governing spec rule without needing to maintain a separate `code`→`ruleId` mapping.

#### Error Codes

| Code                         | ruleId           | Trigger                                          | fixHint                                                                |
| ---------------------------- | ---------------- | ------------------------------------------------ | ---------------------------------------------------------------------- |
| `TOKEN_MISSING`              | `RULE-MCPAUTH-1` | No `Authorization` header or `ATC_PILOT_TOKEN`   | Include `Authorization: Bearer <token>` or set `ATC_PILOT_TOKEN`       |
| `TOKEN_INVALID`              | `RULE-MCPAUTH-1` | Signature verification failed                    | Request a new token from the daemon via the session-token endpoint. If the error persists, verify that `ATC_TOKEN_SECRET` matches the daemon's configured secret |
| `TOKEN_EXPIRED`              | `RULE-MCPAUTH-1` | `expiresAt` < current time                       | Request a new token from the daemon via the session-token endpoint     |
| `PILOT_NOT_ON_MANIFEST`      | `RULE-MCPAUTH-2` | `pilotId` not found in the craft's current crew  | Your pilot identity is not on this craft's current manifest. Verify you are connecting to the correct craft, or ask the captain to add your pilotId to the crew |
| `SEAT_INSUFFICIENT_AUTHORITY`| `RULE-MCPAUTH-7` | Seat type lacks permission for the requested tool| Your current seat type lacks authority for this tool. Check the seat authority matrix (§4.10.3) for the required level. If you need elevated access, ask the captain to adjust your seat assignment |
| `CRAFT_NOT_FOUND`            | `RULE-MCPAUTH-2` | Callsign in the token references a missing craft | The craft may have landed or been abandoned; check craft status         |
...
**Relationship with §4.10 authentication errors.** The standalone MCP server's authentication layer (§4.10) uses a structured object format (`{ code, ruleId, message, fixHint }`) per RULE-MCPAUTH-8, rather than the single-line string format defined by RULE-MCP-2. This is intentional: auth errors occur at the token-validation layer before tool execution begins, and the structured format enables machine-parseable error routing. The `ruleId` field in RULE-MCPAUTH-8 provides traceability parity with RULE-MCP-2 — consumers can map any auth error back to its governing spec rule. Implementors handling both error shapes should check for the presence of a `code` field to distinguish auth-layer errors (§4.10) from tool-layer errors (§4.9).
...
| `officer`       | Captain, First Officer                    | `controls_transfer`, `intercom_send` |
...
- **RULE-MCPAUTH-8:** All auth errors MUST include `code`, `ruleId`, `message`, and `fixHint` fields (§2.9).
...
| RULE-MCPAUTH-8 | All auth errors MUST include `code`, `ruleId`, `message`, and `fixHint`. | 2.9     |
