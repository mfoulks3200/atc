# @airtrafficcontrol/core Implementation Plan
...
**Architecture:** Pure functions that validate domain rules and return new state. No mutation of input objects — always return new instances. Throws domain errors from @airtrafficcontrol/errors on rule violations. Imports type definitions from @airtrafficcontrol/types and validation functions from @airtrafficcontrol/validation.

**Tech Stack:** TypeScript 5.8, vitest, pnpm workspaces

---

## Task 1: Project Setup — Update `@airtrafficcontrol/core` package and remove placeholder

### 1.1 Update `packages/core/package.json`

Add workspace dependencies on `@airtrafficcontrol/types`, `@airtrafficcontrol/errors`, and `@airtrafficcontrol/validation`.

**File:** `packages/core/package.json`

```json
{
  "name": "@airtrafficcontrol/core",
  "version": "0.1.0",
  "private": true,
  "type": "module",
  "main": "./dist/index.js",
  ...
  "dependencies": {
    ...
    "@airtrafficcontrol/validation": "workspace:*"
  }
}
```
...
import type { Vector, FlightPlan } from "@airtrafficcontrol/types";
import {
  getNextVector,
  reportVector,
  allVectorsPassed,
...
**All functions are pure** — they return new objects and never mutate inputs. Domain errors from `@airtrafficcontrol/errors` are thrown with the relevant `ruleId` for traceability.
