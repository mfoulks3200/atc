...
import { useProject, usePilots, useCreateCraft } from "@/hooks/use-api.js";
import { PageHeader } from "@/components/base/page-header.js";
import { SpecImportPanel } from "@/components/forms/spec-import-panel.js";
  ...
  return (
    <div>
      ...
      <div className="mx-auto mt-5 max-w-2xl">
        ...
        >
          {/* Mode tabs */}
          <div className="mb-5 flex gap-0.5 rounded-md p-0.5" style={{ backgroundColor: "var(--bg-elevated)" }}>
            {(["manual", "import"] as CreateMode[]).map((m) => (
              <button
                key={m}
                type="button"
                data-testid={`tab-${m}`}
                onClick={() => setMode(m)}
                className="flex-1 rounded-sm py-1.5 text-[10px] uppercase tracking-widest transition-colors"
                style={{
          ...
          {mode === "import" ? (
            <SpecImportPanel
              project={project!}
              onSuccess={(craft) => navigate(`/projects/${project}/crafts/${craft.callsign}`)}
            />
          ) : (
            <>
              {/* Basic Info */}
