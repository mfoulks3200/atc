...
- `BlackBoxEntryType.Merge`, `MergeStale`, `MergeConflict` — recorded by the tower when executing the merge protocol. `Merge` marks a successful land into main; `MergeStale` and `MergeConflict` record the reasons a craft was returned to GoAround. @see RULE-TMRG-2, RULE-TMRG-3
- `BlackBoxEntryType.AgentOutput` — captured stdout/stderr line from a piloting agent's subprocess, distinct from `Observation` (which is a deliberate log entry authored by an agent). Recorded by the daemon's output pipe when streaming subprocess I/O into the black box and per-craft WebSocket channel.
