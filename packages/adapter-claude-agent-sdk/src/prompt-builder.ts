...
export function buildSystemPrompt(
  ...
  return [
    ...
    "",
    "**To request exclusive controls (two-step: announce on intercom, then claim):**",
    "",
    "1. Broadcast the request via `intercom_send`:",
    "```",
    `// intercom_send({content: "{Holder}, ${pilotId} — requesting controls. My controls?"})`,
    "```",
    "",
    "2. After acknowledgment, take the controls via the API:",
    "```bash",
    `curl -X POST ${craftBase}/controls/claim \\`,
    `  -H "Content-Type: application/json" \\`,
    `  -d '{"pilotId":"${pilotId}"}'`,
    "```",
    "",
